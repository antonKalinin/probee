...
impl KeyEventsState {
    ...
    pub fn push(&mut self, key_event: KeyEvent) -> bool {
...
impl HotKey {
    ...
    pub fn to_keystroke(&self) -> String {
        if self.double_pressed {
            format!("{}+{}", self.key.to_keystroke(), self.key.to_keystroke())
        } else {
            let mods_str = self
                .mods
                .iter()
                .map(|m| m.to_keystroke())
                .collect::<Vec<_>>()
                .join("+");
    ...
    pub fn init(cx: &mut App) {
        ...
        cx.spawn(async move |cx| {
            ...
            loop {
                ...
                let pressed = state.push(event);
                let hotkey = state.into_hotkey();

                if pressed && hotkey.is_some() {
                    let _ = cx.update(|cx| {
